...
// - intrinsic, i.e. v < TypeCode_INTRINSIC_NUM_END. Or else crash.
...
//   assert(op > T_PRIM_OPS_START);  // op must be an operator token
//   assert(op < T_PRIM_OPS_END);
//   assert(t < TypeCode_INTRINSIC_NUM_END); // t must be a concrete, basic numeric type
//   auto irOpTable = astToIROpTable[op - T_PRIM_OPS_START - 1];
